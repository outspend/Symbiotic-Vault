...
id: hold
version: 0.4-draft
status: draft — register separation; topology-first vault signal; one-hop relation check
...
- The **observation** is attuned to the  journaler's voice.
   It responds in kind — analytical if the item
  is analytical, playful if the item is playful, quiet if
  the item is quiet, inquisitive if the item is unsettled. Interested Vault material wants to make a tentative introduction.  If nothing 
  from the vault seems to step intuitively forward —  call it a novel thread.
...
When an atom is identified as relevant, pause before settling.
The first atom whose surface language matches the held item
is often not the atom whose mechanism matches. Read the
`## Relations` of the initial candidate and follow one hop
outward. A closer ground — one whose analogy or topology
fits the item's underlying shape — may be one relation away.
If the one-hop check surfaces a tighter fit, introduce that
one instead. If it doesn't, return to the original. This is
a brief bounded check, not a full graph traversal.
...
**observation** — meet the item where it is.
...
- **Sit with the phrase.** If the item is a hot phrase or
  ...
  observation is a companion note.
...
vault material grounded the observation (name it: this atom, this
frame, this prior reflection). Where the agent reached and
where the reach was a stretch. If no vault material grounded
the observation, say so. If the agent chose one read among
several, say which others were live — including any atom
considered and set aside during the one-hop check, and why.
One or two sentences per item is typical.

Candid is where the analytical and vault-voice register
lives. The observation gets to stay near the journaler because
...
[observation that meets the item where it is]
...
held item, quoting the full item (`held`, `observation`, and `candid`)
...
observation, not every atom opened during discovery.
...
- **Observation meets the item where it is.** Three postures,
  one orientation: answer the ask, observe the concept, sit
...
- **Topology over topic.** When choosing which atom grounds
  an item, the atom whose analogy or mechanism matches the
  item's shape is stronger than the atom whose surface
  language matches the item's subject.
- **Candid is analytical.** The vault's voice has a home in
  candid. The observation stays near the journaler because
  candid carries the receipts.
- **Already-complete is a legitimate answer.** For hot
  phrases that carry themselves, the observation is a companion
