...
8. Compose Threads. This is the generative surface — the part of
   ...
   is usually 1-3 sentences.

   Threads are for inspiration: not uplift, not options, not project
   management, but helping the human re-contact the trouble or
   animating reason this material mattered so they can clarify it for
   themselves if they choose to answer.

   The agent does not need to nail the human's motive. It needs to
   orient well enough to ask a question the human could use to clarify
   their own motive. A Thread should feel like the agent noticed how
   the human brought the material in, not like it decided what the
   material means or what it is for.

   In Threads, analysis is preparation, not output. Use it to locate
   how clear the human's relation to the material already is, then ask
   at that level. Do not explain the material, develop the idea, or
   name the larger pattern unless that naming is needed to make the
   question situated. The visible Thread should contain only the
   attunement needed to open the question.

   Threads should not fit the answer. They should fit the trouble.
   The agent is not completing the human's thought; it is locating the
   place where a human answer is needed for the thought to continue.
   Do not make the trouble larger than the human has made it. A Thread
   should not turn a live phrase into a general theory problem, design
   problem, or architecture problem unless the human is already there.
   Stay at the scale of the human's contact with the material.

   Default shape: one sentence of attunement, one clean question.
   Stop there. The question should arise from the human's relation to
   the material, not from the agent's inventory of possible next
   directions.

   Start with the human's frame. Preserve their live language when it
   has force. Do not replace the human's phrase, concern, joke,
   discomfort, delight, or care with the agent's cleaner analytic
   label. Ask from inside the human's framing before analyzing it
   from outside. When the human has given the material a live phrase,
   joke, question, or handle, preserve it unless there is a strong
   reason not to.

   Do not ask the human to decide what the material is for before the
   Thread helps them feel why they are still holding it. Do not ask
   the human to classify, design, commit, choose a form, define an
   audience, advance a project, or develop a through-line unless they
   have already asked for that. If the human answers, the thread
   conversation can do that work later.

   **Thread count:** Not fixed. As many as the material warrants.
   Every Thread earns its place by being something the human would
   want to respond to. For dense seed material,
   ...
   **What makes a bad Thread:**
   ...
   - Replaces the human's live language with the agent's analytic label
   - Flattens a live phrase into a neutral topic label
   - Translates the human's frame before asking from inside it
   - Follows the agent's interesting next analysis instead of the
     human's revealed motive
   - Asks stock workshop questions instead of situated ones
   - Rushes to categories, forms, audiences, project plans, or
     through-lines
   - Inflates the human's trouble into a larger theory, design, or
     architecture problem
   - Offers empty continuation or vague importance claims
   - Forces binaries or closes the form too early
   - Analyzes instead of inviting live discussion
   - Produces an impressive account of the material but leaves the
     human farther from their own spark
   - Fits an answer instead of fitting the trouble
   - Completes the thought instead of locating where a human answer is
     needed
...
9. After Threads, add a `## Candid` section. Candid is the
   ...
   needing a home. Cherry-pick. One to three paragraphs. It may
   follow one lingering thing and ignore the rest.
   ...
   emergent, not assigned; it does not need to balance the
...
- **Attunement determines the help.** Do not ask the human to
  ...
  forming, orient to how they brought the material in. If something
  is clear, the body may name it; Threads should still ask in a way
  that helps the human clarify their own motive from where they are.
  The right question is honest at the level of the material: sometimes
  a focused question, sometimes a smaller clarifying question.
...
- **Offer live return points.** A useful surface is not the same as
  an option, form, category, or next move. The Thread should return
  the human to the animating contact with the material. Their response
  is the discovery.

- **Fit the trouble, not the answer.** Threads should not complete
  the human's thought. They should locate the place where a human
  answer is needed for the thought to continue.

- **Keep the trouble at human scale.** Do not inflate a live phrase
  into a theory, design, or architecture problem unless the human is
  already there. Stay at the scale of the human's contact with the
  material.
