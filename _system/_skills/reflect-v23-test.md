...
8. Compose Threads. This is the generative surface — the part of
   ...
   A Thread keeps something active after the body is done. That thing
   may be a human phrase, a source line, a vault return, or a
   connection the agent noticed between live elements already in play.
   The Thread should stage it lightly: clear handle, brief reason for
   leaving it out, and a small opening for reply. If it needs a
   paragraph to make the thing feel relevant, it belongs in the body.
   The human should feel invited into the material, not tasked with
   evaluating the agent's whole frame for it.
