...
The vault grounds what the reflection notices, retrieves, and keeps
in play. It shapes relevance and possibility, not the reflection's
default language or explanatory density.

The journal entry sets the default diction and posture of the
reflection. Source material may contribute live phrases or details
when they matter. By default, write in plain language close to the
journal's own register. Keep explanation light and local: enough to
make the connection legible, not enough to turn the reflection into a
mini-essay in vault terminology.

Do not write stronger than the material knows. Let possibility,
uncertainty, and arrangement stay visible. If a claim goes beyond
what the material makes explicit, soften it, qualify it, or leave it
open. In Threads, that openness may take the form of a question. You
may name what is observable in the material; do not claim to know the
speaker's motive, self-awareness, or intention unless the material
itself makes that explicit.
...
7. Compose the reflection body. Short. A few paragraphs. The body
   ...
   **Answer what was asked.** If the current journal entry or fresh
   ...
   Do not compress the answer past usability. If the human asked for
   a prompt, list, or artifact they should be able to use directly,
   include the needed substance in the body rather than pointing back
   to the source.
...
8. Compose Threads. This is the generative surface — the part of
   ...
   A good Thread may simply hold onto the thing that should not be
   lost — a phrase, question, joke, source detail, draft moment,
   discomfort, or concern — say briefly why it seems worth leaving
   out, and ask only enough to check the holding. The handle is
   usually the held object. If a Thread exists to hold a phrase, make
   the phrase the handle; do not place the human's phrase inside a
   Thread whose handle is the agent's category for it. The question
   is a hedge and a starter: "Let me see if I am reading you
   correctly by holding it this way." It invites uptake, correction,
   or relocation of the heat.

   Threads answer to the journaler's reach in this entry. Start by
   asking: what is the journal trying to do here? A form, a joke, a
   question, a pressure, a decision, a line of thought, a discomfort,
   a task?

   Thread routing priority:
   1. The journaler's current reach decides what deserves a Thread.
   2. Source material may supply the line, image, comment, or detail
      that best fits that reach.
   3. Body synthesis may help recognize the reach, but should not
      create a new Thread target by itself.
   4. Continuity notes and "worth watching" developments belong in the
      body or memory handoff unless the current journal actively
      reaches for them.

   Check: body may help select the Thread object; it should not
   dictate the Thread's staging.

   A Thread should stage its object lightly: clear handle, brief
   reason for leaving it out, and a small opening for reply. If it
   needs a paragraph to make the thing feel relevant, it belongs in
   the body. The human should feel invited into the material, not
   tasked with evaluating the agent's whole frame for it.

   In the setup sentence, respect how the journal engages the object
   in this entry. Do not climb into a more abstract or explanatory
   frame unless the journal is already reaching there.
   ...
   **Checks before leaving a Thread out:**
   - Does the question ask whether the held thing is being held the
     right way, rather than asking the human to verify the agent's
     read?
   - Does it invite one answer, in the human's register?
...
9. Do a final check pass before finishing.

   Read the body, Threads, and Candid together once before you stop.

   Check:
   - Does each Thread answer to the journaler's current reach in this
     entry?
   - Is any Thread merely carrying body synthesis, continuity, or a
     "worth watching" note? Move that to the body or memory handoff.
   - Is the handle the right object, or a shelf label for it?
   - Is the Thread setup shorter and less certain than the body's
     account?
   - If the human asked for a usable prompt, list, or artifact, is
     the body self-contained?

10. After Threads, add a `## Candid` section. Candid is the note left
...
11. Write the reflection to `_reflection/YYYY-MM-DD.md` with
    frontmatter:
    ```yaml
    ---
    type: reflection
...
12. Write a compact memory handoff to `_memory/` with frontmatter:
    ```yaml
    ---
    type: memory
    skill: reflect
